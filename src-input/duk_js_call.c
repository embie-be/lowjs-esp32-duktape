...
DUK_LOCAL DUK_NOINLINE void
duk__call_c_recursion_limit_check_slowpath(duk_hthread *thr)
{
    /* When augmenting an error, the effective limit is a bit higher.
     * Check for it only if the fast path check fails.
     */
#if defined(DUK_USE_AUGMENT_ERROR_THROW) ||                                    \
    defined(DUK_USE_AUGMENT_ERROR_CREATE)
    if(thr->heap->augmenting_error)
    {
        if(thr->heap->call_recursion_depth <
           thr->heap->call_recursion_limit + DUK__AUGMENT_CALL_RELAX_COUNT)
        {
            DUK_D(DUK_DPRINT("C recursion limit reached but augmenting error "
                             "and within relaxed limit"));
            return;
        }
    }
#endif
...
DUK_LOCAL DUK_ALWAYS_INLINE void
duk__call_c_recursion_limit_check(duk_hthread *thr)
{
     ...
     */
    if(DUK_LIKELY(thr->heap->call_recursion_depth <
                  thr->heap->call_recursion_limit))
    {
        return;
    }
...
DUK_LOCAL DUK_NOINLINE void
duk__call_callstack_limit_check_slowpath(duk_hthread *thr)
{
    if(thr->sent_fatal_throw) // enough to fix infinite recursion problem.. but
                              // might not be enough if we catch other errors
                              // later
        return;
    ...
    if(thr->heap->augmenting_error)
    {
        if(thr->callstack_top <
           DUK_USE_CALLSTACK_LIMIT + DUK__AUGMENT_CALL_RELAX_COUNT)
        {
            DUK_D(DUK_DPRINT("call stack limit reached but augmenting error "
                             "and within relaxed limit"));
            return;
        }
    }
...
DUK_LOCAL DUK_ALWAYS_INLINE void
duk__call_callstack_limit_check(duk_hthread *thr)
{
    /* This check is forcibly inlined because it's very cheap and almost
     * always passes.  The slow path is forcibly noinline.
     */
...
DUK_LOCAL void duk__interrupt_fixup(duk_hthread *thr,
                                    duk_hthread *entry_curr_thread)
{
    ...
    if(entry_curr_thread == NULL)
    {
        ...
        DUK_DD(DUK_DDPRINT(
            "debug test: updated interrupt count on exit to "
...
DUK_LOCAL void duk__create_arguments_object(duk_hthread *thr, duk_hobject *func,
                                            duk_hobject *varenv,
                                            duk_idx_t idx_args)
{
    duk_hobject *arg; /* 'arguments' */
    duk_hobject
        *formals; /* formals for 'func' (may be NULL if func is a C function) */
    duk_idx_t i_arg;
    duk_idx_t i_map;
    ...
    else
    {
         ...
         */
        DUK_D(DUK_DPRINT("_Formals is undefined when creating arguments, use "
                         "n_formals == 0"));
        n_formals = 0;
    }
    ...
    DUK_DDD(DUK_DDDPRINT(
        "created arguments related objects: "
        "arguments at index %ld -> %!O "
        "map at index %ld -> %!O "
        "mappednames at index %ld -> %!O",
        (long)i_arg, (duk_heaphdr *)duk_get_hobject(thr, i_arg), (long)i_map,
        (duk_heaphdr *)duk_get_hobject(thr, i_map), (long)i_mappednames,
    ...
    while(idx >= 0)
    {
        DUK_DDD(DUK_DDDPRINT("arg idx %ld, argbase=%ld, argidx=%ld", (long)idx,
                             (long)i_argbase, (long)(i_argbase + idx)));
        ...
        if(!DUK_HOBJECT_HAS_STRICT(func) && idx < n_formals)
        {
            ...
            DUK_DDD(DUK_DDDPRINT(
                "strict function, index within formals (%ld < %ld)", (long)idx,
                (long)n_formals));
            ...
            if(!duk_has_prop(thr, i_mappednames))
            {
                ...
                (void)duk_push_uint_to_hstring(thr,
                                               (duk_uint_t)idx); /* index */
                duk_xdef_prop_wec(
                    thr, i_mappednames); /* out of spec, must be configurable */

                DUK_DDD(DUK_DDDPRINT("set map[%ld]=%s", (long)idx,
    ...
    if(need_map)
    {
        ...
        duk_xdef_prop_stridx(
            thr, i_arg, DUK_STRIDX_INT_VARENV,
            DUK_PROPDESC_FLAGS_NONE); /* out of spec, don't care */
    }
    ...
    {
        DUK_DDD(DUK_DDDPRINT(
            "non-strict function, setting callee to actual value"));
        duk_push_hobject(thr, func);
        duk_xdef_prop_stridx(thr, i_arg, DUK_STRIDX_CALLEE,
    ...
    {
        DUK_DDD(
            DUK_DDDPRINT("not enabling exotic behavior for arguments object"));
    }

    DUK_DDD(DUK_DDDPRINT(
        "final arguments related objects: "
        "arguments at index %ld -> %!O "
        "map at index %ld -> %!O "
        "mappednames at index %ld -> %!O",
        (long)i_arg, (duk_heaphdr *)duk_get_hobject(thr, i_arg), (long)i_map,
                                               ...
                                               duk_idx_t idx_args)
{
    ...
    duk_xdef_prop_stridx_short(
        thr, -2, DUK_STRIDX_LC_ARGUMENTS,
        DUK_HOBJECT_HAS_STRICT(func) ? DUK_PROPDESC_FLAGS_E
                                     : /* strict: non-deletable, non-writable */
                                                  ...
                                                  duk_idx_t idx_func)
{
    ...
    {
        DUK_DDD(DUK_DDDPRINT(
            "constructor has no 'prototype' property, or value not an object "
            "-> leave standard Object prototype as fallback prototype"));
    }
    else
    {
        DUK_DDD(DUK_DDDPRINT(
            "constructor has 'prototype' property with object value "
...
DUK_INTERNAL void
duk_call_construct_postprocess(duk_hthread *thr,
    ...
    if(DUK_TVAL_IS_OBJECT(tv_func))
    {
        ...
        if(DUK_HOBJECT_HAS_BOUNDFUNC(func))
        {
            ...
            if(is_constructor_call)
            {
                ...
                DUK_DDD(DUK_DDDPRINT(
                    "constructor call: don't update this binding"));
            }
            else
            {
            ...
            DUK_DDD(DUK_DDDPRINT(
                "bound function handled, idx_func=%ld, curr func=%!T",
...
DUK_LOCAL duk_bool_t duk__handle_specialfuncs_for_call(
    duk_hthread *thr, duk_idx_t idx_func, duk_hobject *func,
    duk_small_uint_t *call_flags, duk_bool_t first)
{
#if defined(DUK_USE_ASSERTIONS)
    ...
    if(DUK_UNLIKELY(((duk_hnatfunc *)func)->magic == 15))
    {
        ...
        {
            *call_flags = (*call_flags & ~DUK_CALL_FLAG_CALLED_AS_EVAL) |
    ...
    { /* 0=Function.prototype.call() */
         ...
         * idx_func + 3: 2nd argument to .call, desired 1st argument for
         * ultimate target
         * ...
         *
        ...
        if(thr->valstack_top < tv_args)
        {
            ...
            thr->valstack_top = tv_args; /* at least target function and 'this'
                                            binding present */
        }
        break;
    }
    ...
    { /* 2=Reflect.apply() */
         ...
         * [removed] idx_func + 2: 1st argument to .apply, target function
    ...
    { /* 3=Reflect.construct() */
         ...
         * [removed] idx_func + 2: 1st argument to .construct, target function
        ...
        *call_flags |= DUK_CALL_FLAG_CONSTRUCT;
        ...
        if(top > idx_func + 3)
        {
            ...
            duk_set_top_unsafe(
                thr, idx_func + 3); /* remove any args beyond argArray */
        }
        DUK_ASSERT(duk_get_top(thr) == idx_func + 3);
        DUK_ASSERT(duk_is_valid_index(thr, idx_func + 2));
        (void)duk_unpack_array_like(
            thr, idx_func + 2); /* XXX: should also remove target to be
                                   symmetric with duk_pack()? */
...
apply_shared:
    ...
    if(thr->valstack_top <= tv_args)
    {
        DUK_ASSERT(tv_args <= thr->valstack_end);
        thr->valstack_top =
            tv_args; /* at least target func and 'this' binding present */
                     /* No need to check for argArray. */
    }
    else
    {
        DUK_ASSERT(duk_get_top(thr) >=
                   idx_func + 3); /* idx_func + 2 covered above */
...
DUK_LOCAL void duk__handle_proxy_for_call(duk_hthread *thr, duk_idx_t idx_func,
                                          duk_hproxy *h_proxy,
                                          duk_small_uint_t *call_flags)
{
    ...
    rc = duk_get_prop_stridx_short(thr, -1,
                                   (*call_flags & DUK_CALL_FLAG_CONSTRUCT)
    ...
    {
        *call_flags |=
            DUK_CALL_FLAG_CONSTRUCT_PROXY; /* Enable 'construct' trap return
                                              invariant check. */
        *call_flags &= ~(DUK_CALL_FLAG_CONSTRUCT); /* Resume as non-constructor
                                                      call to the trap. */
...
DUK_LOCAL void duk__handle_oldenv_for_call(duk_hthread *thr, duk_hobject *func,
                                           duk_activation *act)
{
    duk_hcompfunc *f;
    duk_hobject *h_lex;
    ...
    DUK_ASSERT(h_lex !=
               NULL); /* Always true for closures (not for templates) */
...
DUK_LOCAL void duk__update_func_caller_prop(duk_hthread *thr, duk_hobject *func)
{
    ...
    if(tv_caller)
    {
        ...
        if(DUK_TVAL_IS_OBJECT(tv_caller))
        {
            ...
            /* Previous value doesn't need refcount changes because its
             * ownership is transferred to prev_caller.
             */
                                                             ...
                                                             duk_idx_t idx_this)
{
    ...
    case DUK_TAG_NULL:
        DUK_DDD(DUK_DDDPRINT(
            "this binding: non-strict, undefined/null -> use global object"));
        ...
        if(DUK_LIKELY(obj_global != NULL))
        {
            DUK_ASSERT(!DUK_TVAL_IS_HEAP_ALLOCATED(
                tv_this)); /* no need to decref previous value */
        ...
        else
        {
             ...
             */
            DUK_D(DUK_DPRINT("this binding: wanted to use global object, but "
                             "it is NULL -> using undefined instead"));
            DUK_ASSERT(!DUK_TVAL_IS_HEAP_ALLOCATED(
                tv_this)); /* no need to decref previous value */
...
DUK_LOCAL DUK_ALWAYS_INLINE duk_bool_t duk__resolve_target_fastpath_check(
    duk_hthread *thr, duk_idx_t idx_func, duk_hobject **out_func,
    duk_small_uint_t call_flags)
{
...
DUK_LOCAL duk_hobject *
duk__resolve_target_func_and_this_binding(duk_hthread *thr, duk_idx_t idx_func,
                                          duk_small_uint_t *call_flags)
{
    duk_tval *tv_func;
    duk_hobject *func;
    ...
    for(first = 1;; first = 0)
    {
        ...
        if(DUK_TVAL_IS_OBJECT(tv_func))
        {
            ...
            /* XXX: could set specialcall for boundfuncs too, simplify check
             * above */
            ...
            if(DUK_HOBJECT_HAS_BOUNDFUNC(func))
            {
                ...
                duk__handle_bound_chain_for_call(
                    thr, idx_func, *call_flags & DUK_CALL_FLAG_CONSTRUCT);

                DUK_ASSERT(
                    DUK_TVAL_IS_OBJECT(duk_require_tval(thr, idx_func)) ||
            ...
            else
            {
                ...
                if(DUK_HOBJECT_HAS_EXOTIC_PROXYOBJ(func))
                {
                     ...
                     */
                    duk__handle_proxy_for_call(thr, idx_func,
                                               (duk_hproxy *)func, call_flags);
                }
                else
#endif
                {
                    ...
                    if(duk__handle_specialfuncs_for_call(
                           thr, idx_func, func, call_flags, first) != 0)
                    {
...
#if defined(DUK_USE_ASSERTIONS)
{
    ...
    DUK_ASSERT((DUK_TVAL_IS_OBJECT(tv_tmp) &&
                DUK_HOBJECT_IS_CALLABLE(DUK_TVAL_GET_OBJECT(tv_tmp))) ||
               DUK_TVAL_IS_LIGHTFUNC(tv_tmp));
    DUK_ASSERT(func == NULL || !DUK_HOBJECT_HAS_BOUNDFUNC(func));
    DUK_ASSERT(func == NULL ||
               (DUK_HOBJECT_IS_COMPFUNC(func) || DUK_HOBJECT_IS_NATFUNC(func)));
...
not_callable:
    ...
    if(DUK_TVAL_IS_OBJECT(tv_func))
    {
        if(duk_hobject_find_existing_entry_tval_ptr(
               thr->heap, DUK_TVAL_GET_OBJECT(tv_func),
               DUK_HTHREAD_STRING_INT_TARGET(thr)) != NULL)
        {
    ...
    DUK_ERROR_FMT1(thr, DUK_ERR_TYPE_ERROR, "%s not callable",
                   duk_get_type_name(thr, idx_func));
#else
    DUK_ERROR_FMT1(thr, DUK_ERR_TYPE_ERROR, "%s not callable",
                   duk_push_string_tval_readable(thr, tv_func));
#endif
#else
...
#if defined(DUK_USE_VERBOSE_ERRORS)
#if defined(DUK_USE_PARANOID_ERRORS)
    DUK_ERROR_FMT1(thr, DUK_ERR_TYPE_ERROR, "%s not constructable",
                   duk_get_type_name(thr, idx_func));
#else
    DUK_ERROR_FMT1(thr, DUK_ERR_TYPE_ERROR, "%s not constructable",
                   duk_push_string_tval_readable(thr, tv_func));
                                              ...
                                              duk_idx_t num_actual_rets)
{
    ...
    idx_rcbase =
        duk_get_top(thr) - num_actual_rets; /* base of known return values */
    ...
    DUK_DDD(DUK_DDDPRINT("adjust valstack after func call: "
                         ...
                         (long)duk_get_top(thr), (long)idx_retbase,
                         (long)idx_rcbase));
    ...
    if(idx_rcbase > idx_retbase)
    {
        ...
        DUK_DDD(DUK_DDDPRINT(
            "elements at/after idx_retbase have enough to cover func retvals "
    ...
    else
    {
        ...
        DUK_DDD(DUK_DDDPRINT(
            "not enough elements at/after idx_retbase to cover func retvals "
            "(idx_retbase=%ld, idx_rcbase=%ld)",
            (long)idx_retbase, (long)idx_rcbase));
...
DUK_LOCAL duk_small_uint_t duk__call_setup_act_attempt_tailcall(
    duk_hthread *thr, duk_small_uint_t call_flags, duk_idx_t idx_func,
    duk_hobject *func, duk_size_t entry_valstack_bottom_byteoff,
    duk_size_t entry_valstack_end_byteoff, duk_idx_t *out_nargs,
    duk_idx_t *out_nregs, duk_size_t *out_vs_min_bytes,
    duk_activation **out_act)
{
    ...
    *out_act = act;
    ...
    {
        DUK_DDD(DUK_DDDPRINT(
            "tail call prevented by target not being ecma function"));
        return 0;
    }
     ...
     */
    flags1 =
        (duk_small_uint_t)((act->flags & DUK_ACT_FLAG_CONSTRUCT) ? 1 : 0)
#if defined(DUK_USE_ES6_PROXY)
        |
    ...
    {
        DUK_DDD(DUK_DDDPRINT(
            "tail call prevented by incompatible return value handling"));
        return 0;
    }
    DUK_ASSERT(((act->flags & DUK_ACT_FLAG_CONSTRUCT) &&
                (call_flags & DUK_CALL_FLAG_CONSTRUCT)) ||
               (!(act->flags & DUK_ACT_FLAG_CONSTRUCT) &&
                !(call_flags & DUK_CALL_FLAG_CONSTRUCT)));
    DUK_ASSERT(((act->flags & DUK_ACT_FLAG_CONSTRUCT_PROXY) &&
                (call_flags & DUK_CALL_FLAG_CONSTRUCT_PROXY)) ||
     ...
     *  because there may be non-error-catching label entries in valid tail
     * calls.
     *
     ...
     *      the executor pause check won't trigger due to shared activation,
     * see: https://github.com/svaarala/duktape/issues/1726.
     */

    DUK_DDD(DUK_DDDPRINT(
        "is tail call, reusing activation at callstack top, at index %ld",
    ...
    DUK_ASSERT(DUK_HOBJECT_HAS_COMPFUNC(func));
    act->func =
        func; /* don't want an intermediate exposed state with func == NULL */
#if defined(DUK_USE_NONSTD_FUNC_CALLER_PROPERTY)
    act->prev_caller = NULL;
#endif
    /* don't want an intermediate exposed state with invalid pc */
    act->curr_pc =
        DUK_HCOMPFUNC_GET_CODE_BASE(thr->heap, (duk_hcompfunc *)func);
#if defined(DUK_USE_DEBUGGER_SUPPORT)
    act->prev_line = 0;
#endif
    ...
    act->bottom_byteoff =
        entry_valstack_bottom_byteoff; /* tail call -> reuse current "frame" */
#if 0
    ...
    DUK_ASSERT(tv1 >= thr->valstack &&
               tv1 < thr->valstack_top); /* tv1 is -below- valstack_bottom */
    ...
    idx_func = 0;
    DUK_UNREF(idx_func); /* really 'not applicable' anymore, should not be
...
DUK_LOCAL void duk__call_setup_act_not_tailcall(
    duk_hthread *thr, duk_small_uint_t call_flags, duk_idx_t idx_func,
    duk_hobject *func, duk_size_t entry_valstack_bottom_byteoff,
    duk_size_t entry_valstack_end_byteoff, duk_idx_t *out_nargs,
    duk_idx_t *out_nregs, duk_size_t *out_vs_min_bytes,
    duk_activation **out_act)
{
    ...
    if(act != NULL)
    {
        ...
        act->retval_byteoff = entry_valstack_bottom_byteoff +
                              (duk_size_t)idx_func * sizeof(duk_tval);
    }
    ...
    *out_act = act;
    ...
    DUK_ASSERT(thr->valstack_top >
               thr->valstack_bottom); /* at least effective 'this' */
    DUK_ASSERT(func == NULL || !DUK_HOBJECT_HAS_BOUNDFUNC(func));
    ...
    if(DUK_LIKELY(func != NULL))
    {
        ...
        if(DUK_HOBJECT_IS_COMPFUNC(func))
        {
            ...
            *out_vs_min_bytes =
                ...
                sizeof(duk_tval) *
                    ((duk_size_t)idx_func + 2U + (duk_size_t)*out_nregs +
                     DUK_VALSTACK_INTERNAL_EXTRA);
        }
        else
        {
            ...
            *out_nregs = *out_nargs;
            ...
            else
            {
                ...
                duk_size_t valstack_top_byteoff =
                    (duk_size_t)((duk_uint8_t *)thr->valstack_top -
                                 ((duk_uint8_t *)thr->valstack));
                *out_vs_min_bytes =
                    valstack_top_byteoff +
                    sizeof(duk_tval) * (DUK_VALSTACK_API_ENTRY_MINIMUM +
                                        DUK_VALSTACK_INTERNAL_EXTRA);
            }
        }
    }
        ...
        *out_nargs = DUK_LFUNC_FLAGS_GET_NARGS(lf_flags);
        if(*out_nargs != DUK_LFUNC_NARGS_VARARGS)
        {
            *out_vs_min_bytes =
                entry_valstack_bottom_byteoff +
                sizeof(duk_tval) *
                    ((duk_size_t)idx_func + 2U + (duk_size_t)*out_nargs +
                     DUK_VALSTACK_API_ENTRY_MINIMUM +
                     DUK_VALSTACK_INTERNAL_EXTRA);
        }
        else
    ...
    act->bottom_byteoff = entry_valstack_bottom_byteoff +
                          sizeof(duk_tval) * ((duk_size_t)idx_func + 2U);
#if 0
...
DUK_LOCAL void duk__call_env_setup(duk_hthread *thr, duk_hobject *func,
                                   duk_activation *act, duk_idx_t idx_args)
{
    duk_hobject *env;
    ...
    if(DUK_LIKELY(func != NULL))
    {
        if(DUK_LIKELY(DUK_HOBJECT_HAS_NEWENV(func)))
        {
            ...
            else
            {
                ...
                /* third arg: absolute index (to entire valstack) of
                 * bottom_byteoff of new activation */
                env = duk_create_activation_environment_record(
                    thr, func, act->bottom_byteoff);
                DUK_ASSERT(env != NULL);
                ...
                DUK_HOBJECT_INCREF(thr,
                                   env); /* XXX: incref by count (2) directly */
                duk_pop(thr);
            }
        }
        else
...
thread_state_error:
    DUK_ERROR_FMT1(thr, DUK_ERR_TYPE_ERROR, "invalid thread state (%ld)",
                   (long)thr->state);
    DUK_UNREACHABLE();
}
...
DUK_LOCAL duk_int_t duk__handle_call_raw(duk_hthread *thr, duk_idx_t idx_func,
                                         duk_small_uint_t call_flags)
{
#if defined(DUK_USE_ASSERTIONS)
    ...
    if(call_flags & DUK_CALL_FLAG_TAILCALL)
    {
        ...
        DUK_ASSERT(
            DUK_HOBJECT_IS_COMPFUNC(DUK_ACT_GET_FUNC(thr->callstack_curr)));
        ...
        {
            DUK_ASSERT(DUK_CAT_GET_TYPE(tmp_cat) ==
                       DUK_CAT_TYPE_LABEL); /* a non-catching entry */
        }
    }
...
#endif
    entry_valstack_bottom_byteoff = (duk_size_t)(
        (duk_uint8_t *)thr->valstack_bottom - (duk_uint8_t *)thr->valstack);
    entry_valstack_end_byteoff = (duk_size_t)((duk_uint8_t *)thr->valstack_end -
                                              (duk_uint8_t *)thr->valstack);
    ...
    DUK_DD(DUK_DDPRINT(
        "duk__handle_call_raw: thr=%p, idx_func=%ld, "
        ...
        (void *)thr, (long)idx_func, (unsigned long)call_flags,
        (long)((call_flags & DUK_CALL_FLAG_CONSTRUCT) != 0 ? 1 : 0),
        (long)duk_get_top(thr), (long)idx_func, (long)(idx_func + 2),
        (long)thr->heap->call_recursion_depth,
        (long)thr->heap->call_recursion_limit,
        (long)entry_valstack_bottom_byteoff, (long)entry_valstack_end_byteoff,
        (long)entry_call_recursion_depth, (void *)entry_curr_thread,
    ...
    if(DUK_LIKELY(duk__resolve_target_fastpath_check(thr, idx_func, &func,
                                                     call_flags) != 0U))
    {
    ...
    else
    {
        ...
        func = duk__resolve_target_func_and_this_binding(thr, idx_func,
                                                         &call_flags);
    }
    DUK_ASSERT(duk_get_top(thr) - idx_func >=
               2); /* at least func and this present */
    ...
    if(use_tailcall)
    {
        use_tailcall = duk__call_setup_act_attempt_tailcall(
            thr, call_flags, idx_func, func, entry_valstack_bottom_byteoff,
            entry_valstack_end_byteoff, &nargs, &nregs, &vs_min_bytes, &act);
    }
#else
    DUK_ASSERT((call_flags & DUK_CALL_FLAG_TAILCALL) ==
               0); /* compiler ensures this */
    use_tailcall = 0;
#endif
    ...
    else
    {
        ...
        {
            DUK_ASSERT(nregs >= nargs);
            duk_set_top_and_wipe(thr, idx_func + 2 + nregs,
                                 idx_func + 2 + nargs);
        }
        else
        {
    ...
    {
        duk_call_construct_postprocess(thr, call_flags &
    ...
    DUK_ASSERT((duk_uint8_t *)thr->valstack + entry_valstack_end_byteoff >=
               (duk_uint8_t *)thr->valstack_top);
    DUK_ASSERT((duk_uint8_t *)thr->valstack + entry_valstack_end_byteoff <=
               (duk_uint8_t *)thr->valstack_alloc_end);
...
DUK_INTERNAL duk_int_t duk_handle_call_unprotected_nargs(
    duk_hthread *thr, duk_idx_t nargs, duk_small_uint_t call_flags)
{
    duk_idx_t idx_func;
...
DUK_LOCAL void duk__handle_safe_call_inner(
    duk_hthread *thr, duk_safe_call_function func, void *udata,
#if defined(DUK_USE_ASSERTIONS)
    duk_size_t entry_valstack_bottom_byteoff, duk_size_t entry_callstack_top,
#endif
    duk_hthread *entry_curr_thread, duk_uint_fast8_t entry_thread_state,
    duk_idx_t idx_retbase, duk_idx_t num_stack_rets)
{
    duk_ret_t rc;
    ...
    /* we're running inside the caller's activation, so no change in call/catch
     * stack or valstack bottom */
    DUK_ASSERT(thr->callstack_top == entry_callstack_top);
    DUK_ASSERT(thr->valstack_bottom >= thr->valstack);
    DUK_ASSERT((duk_size_t)((duk_uint8_t *)thr->valstack_bottom -
                            (duk_uint8_t *)thr->valstack) ==
               entry_valstack_bottom_byteoff);
    DUK_ASSERT(thr->valstack_top >= thr->valstack_bottom);
    DUK_ASSERT(thr->valstack_end >= thr->valstack_top);
    ...
    duk__safe_call_adjust_valstack(thr, idx_retbase, num_stack_rets,
                                   rc); /* throws for insane rc */

    DUK_HEAP_SWITCH_THREAD(thr->heap, entry_curr_thread); /* may be NULL */
    thr->state = (duk_uint8_t)entry_thread_state;
}

DUK_LOCAL void duk__handle_safe_call_error(
    duk_hthread *thr, duk_activation *entry_act,
#if defined(DUK_USE_ASSERTIONS)
    duk_size_t entry_callstack_top,
#endif
    duk_hthread *entry_curr_thread, duk_uint_fast8_t entry_thread_state,
    duk_idx_t idx_retbase, duk_idx_t num_stack_rets,
    duk_size_t entry_valstack_bottom_byteoff, duk_jmpbuf *old_jmpbuf_ptr)
{
    DUK_ASSERT(thr != NULL);
    DUK_ASSERT_CTX_VALID(thr);
    ...
    DUK_DD(
        DUK_DDPRINT("safe call error handled, pf_prevent_count updated to %ld",
                    (long)thr->heap->pf_prevent_count));
...
DUK_LOCAL void duk__handle_safe_call_shared_unwind(
    duk_hthread *thr, duk_idx_t idx_retbase, duk_idx_t num_stack_rets,
#if defined(DUK_USE_ASSERTIONS)
    duk_size_t entry_callstack_top,
#endif
    duk_int_t entry_call_recursion_depth, duk_hthread *entry_curr_thread,
    duk_instr_t **entry_ptr_curr_pc)
{
    DUK_ASSERT(thr != NULL);
    DUK_ASSERT_CTX_VALID(thr);
                                            ...
                                            duk_idx_t num_stack_rets)
{
    ...
    idx_retbase =
        duk_get_top(thr) -
        num_stack_args; /* not a valid stack index if num_stack_args == 0 */
    DUK_ASSERT(idx_retbase >= 0);

    DUK_ASSERT((duk_idx_t)(thr->valstack_top - thr->valstack_bottom) >=
               num_stack_args); /* Caller ensures. */
    DUK_ASSERT(
        (duk_idx_t)(thr->valstack_end - (thr->valstack_bottom + idx_retbase)) >=
        num_stack_rets); /* Caller ensures. */
    ...
    DUK_DD(DUK_DDPRINT(
        ...
        "entry_act=%p, entry_valstack_bottom_byteoff=%ld, "
        "entry_call_recursion_depth=%ld, "
        "entry_curr_thread=%p, entry_thread_state=%ld",
        (void *)thr, (long)num_stack_args, (long)num_stack_rets,
        (long)duk_get_top(thr), (long)idx_retbase,
        (long)thr->heap->call_recursion_depth,
        (long)thr->heap->call_recursion_limit, (void *)entry_act,
        (long)entry_valstack_bottom_byteoff, (long)entry_call_recursion_depth,
        (void *)entry_curr_thread, (long)entry_thread_state));
    ...
    if(DUK_SETJMP(our_jmpbuf.jb) == 0)
    {
        ...
        duk__handle_safe_call_inner(
            thr, func, udata,
#if defined(DUK_USE_ASSERTIONS)
            entry_valstack_bottom_byteoff, entry_callstack_top,
#endif
            entry_curr_thread, entry_thread_state, idx_retbase, num_stack_rets);
         ...
         */
        DUK_ASSERT(thr->callstack_curr == entry_act);
        DUK_ASSERT((duk_size_t)((duk_uint8_t *)thr->valstack_end -
                                (duk_uint8_t *)thr->valstack) >=
                   entry_valstack_end_byteoff);
...
#endif
            entry_curr_thread, entry_thread_state, idx_retbase, num_stack_rets,
            entry_valstack_bottom_byteoff, old_jmpbuf_ptr);
    ...
    catch(std::exception &exc)
    {
        ...
        try
        {
            DUK_ERROR_FMT1(thr, DUK_ERR_TYPE_ERROR,
                           "caught invalid c++ std::exception '%s' (perhaps "
                           "thrown by user code)",
                           what);
        }
        catch(duk_internal_exception exc)
        {
            ...
            duk__handle_safe_call_error(
                ...
                entry_callstack_top,
#endif
                entry_curr_thread, entry_thread_state, idx_retbase,
                num_stack_rets, entry_valstack_bottom_byteoff, old_jmpbuf_ptr);
            retval = DUK_EXEC_ERROR;
        }
    }
    catch(...)
    ...
    DUK_ASSERT(thr->heap->lj.jmpbuf_ptr ==
               old_jmpbuf_ptr); /* success/error path both do this */
    ...
    DUK_ASSERT((duk_size_t)((duk_uint8_t *)thr->valstack_end -
                            (duk_uint8_t *)thr->valstack) >=
               entry_valstack_end_byteoff);
    duk__handle_safe_call_shared_unwind(thr, idx_retbase, num_stack_rets,
#if defined(DUK_USE_ASSERTIONS)
                                        entry_callstack_top,
#endif
                                        entry_call_recursion_depth,
                                        entry_curr_thread, entry_ptr_curr_pc);
...
#if defined(DUK_USE_VERBOSE_ERRORS)
DUK_INTERNAL DUK_NOINLINE DUK_COLD void
duk_call_setup_propcall_error(duk_hthread *thr, duk_tval *tv_targ,
                              duk_tval *tv_base, duk_tval *tv_key)
{
    ...
    duk_push_error_object(
        thr, DUK_ERR_TYPE_ERROR | DUK_ERRCODE_FLAG_NOBLAME_FILELINE,
        "%s not callable (property %s of %s)", str1, str2, str3);
#else
    ...
    duk_push_true(thr);
    duk_put_prop_stridx(
        thr, -2, DUK_STRIDX_INT_TARGET); /* Marker property, reuse _Target. */

    /* [ <nregs> propValue <variable> error ] */
    duk_replace(thr, entry_top - 1);
    duk_set_top(thr, entry_top);

    DUK_ASSERT(!duk_is_callable(
        thr, -1)); /* Critical so that call handling will throw the error. */
}
