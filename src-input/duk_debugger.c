...
unsigned char is_in_user_file(duk_hthread *thr)
{
    if(!thr->callstack_curr)
        return 0;
    duk_push_tval(thr, &thr->callstack_curr->tv_func);
    duk_get_prop_stridx_short(thr, -1, DUK_STRIDX_FILE_NAME);
    const char *fName = duk_get_string_notsymbol(thr, -1);
    unsigned char is = fName && fName[0] == '/';
    duk_pop_2(thr);
    return is;
}
...
{
    // neonious wants additional reads!
}
                                          ...
                                          duk_small_uint_t pause_flags)
{
    ...
    if(line == 0)
    {
        ...
        DUK_D(DUK_DPRINT("no line info for current activation, disable "
                         "line-based pause flags: 0x%08lx -> 0x%08lx",
    ...
    DUK_D(DUK_DPRINT("set state for automatic pause triggers, flags=0x%08lx, "
                     "act=%p, startline=%ld",
                                       ...
                                       duk_size_t length)
{
    ...
    for(;;)
    {
        ...
        if(got == 0 || got > left)
        {
            ...
            duk__debug_null_most_callbacks(
                thr); /* avoid calling write callback in detach1() */
...
DUK_LOCAL duk_uint32_t duk__debug_read_uint32_raw(duk_hthread *thr)
{
    ...
    return ((duk_uint32_t)buf[0] << 24) | ((duk_uint32_t)buf[1] << 16) |
           ((duk_uint32_t)buf[2] << 8) | (duk_uint32_t)buf[3];
}
                                                   ...
                                                   duk_uint32_t len)
{
    ...
    {
        p = (duk_uint8_t *)duk_push_fixed_buffer(
            thr, (duk_size_t)len); /* zero for paranoia */
        DUK_ASSERT(p != NULL);
        duk_debug_read_bytes(thr, p, (duk_size_t)len);
        (void)duk_buffer_to_string(
            thr, -1); /* Safety relies on debug client, which is OK. */
    }
...
/* Write fully. */
DUK_INTERNAL void duk_debug_write_bytes(duk_hthread *thr,
                                        ...
                                        duk_size_t length)
{
    ...
    if(heap->dbg_write_cb == NULL)
    {
        DUK_D(DUK_DPRINT("attempt to write %ld bytes in detached state, ignore",
                         (long)length));
        return;
    }
    if(length == 0)
...
DUK_INTERNAL void duk_debug_write_hstring(duk_hthread *thr, duk_hstring *h)
{
    ...
    duk_debug_write_string(
        thr, (h != NULL ? (const char *)DUK_HSTRING_GET_DATA(h) : NULL),
...
DUK_INTERNAL void duk_debug_write_hbuffer(duk_hthread *thr, duk_hbuffer *h)
{
    ...
    duk_debug_write_buffer(
        thr,
        (h != NULL ? (const char *)DUK_HBUFFER_GET_DATA_PTR(thr->heap, h)
                   : NULL),
                                            ...
                                            duk_uint8_t ibyte)
{
    ...
    duk_byteswap_bytes((duk_uint8_t *)pu.b, sizeof(pu));
#endif
    duk_debug_write_bytes(thr, (const duk_uint8_t *)&pu.p,
                          (duk_size_t)sizeof(pu));
}
...
DUK_INTERNAL void duk_debug_write_tval(duk_hthread *thr, duk_tval *tv)
{
    ...
    default:
        ...
        DUK_DD(DUK_DDPRINT("i32=%ld du1=%02x%02x%02x%02x%02x%02x%02x%02x "
                           ...
                           (long)i32, (unsigned int)du1.uc[0],
                           (unsigned int)du1.uc[1], (unsigned int)du1.uc[2],
                           (unsigned int)du1.uc[3], (unsigned int)du1.uc[4],
                           (unsigned int)du1.uc[5], (unsigned int)du1.uc[6],
                           (unsigned int)du1.uc[7], (unsigned int)du2.uc[0],
                           (unsigned int)du2.uc[1], (unsigned int)du2.uc[2],
                           (unsigned int)du2.uc[3], (unsigned int)du2.uc[4],
                           (unsigned int)du2.uc[5], (unsigned int)du2.uc[6],
        ...
        else
        {
            ...
            duk_debug_write_bytes(thr, (const duk_uint8_t *)du1.uc,
                                  sizeof(du1.uc));
        }
    }
}
...
DUK_INTERNAL void duk_debug_write_error_eom(duk_hthread *thr,
...
DUK_INTERNAL void duk_debug_write_notify(duk_hthread *thr,
...
DUK_INTERNAL void duk_debug_send_status(duk_hthread *thr)
{
    ...
    else
    {
        ...
        duk_debug_write_uint(
            thr, (duk_uint32_t)duk_hthread_get_act_curr_pc(thr, act));
    }
...
{
    /*
     *  NFY <int: 5> <int: fatal> <str: msg> <str: filename> <int: linenumber>
     * EOM
     */
    ...
    if(fatal)
    {
        (void)duk_safe_to_string(thr, -1);
        printf("DukTape fatal throw, paused in IDE: %s\n",
    ...
    if(duk_is_error(thr, -1))
    {
        ...
        duk_uint32_t line = duk_get_uint(thr, -1);
#ifdef CONFIG_DEBUG
        if(fatal)
            printf("Line: %d\n", (int)line);
#endif /* CONFIG_DEBUG */
        duk_debug_write_uint(thr, line);
        duk_pop_2(thr);
    }
    else
    {
        ...
        if(act != NULL)
        {
            ...
            duk_uint32_t line = duk_hobject_pc2line_query(thr, -2, pc);
#ifdef CONFIG_DEBUG
            if(fatal)
                printf("Line: %d\n", (int)line);
#endif /* CONFIG_DEBUG */
...
DUK_LOCAL void duk__debug_handle_trigger_status(duk_hthread *thr,
                                                duk_heap *heap)
{
    DUK_UNREF(heap);
                                      ...
                                      duk_int32_t cmd)
{
    ...
    {
        pause_flags = DUK_PAUSE_FLAG_LINE_CHANGE | DUK_PAUSE_FLAG_FUNC_ENTRY |
...
DUK_LOCAL void duk__debug_handle_add_break(duk_hthread *thr, duk_heap *heap)
{
    ...
    {
        duk_debug_write_error_eom(thr, DUK_DBG_ERR_TOOMANY,
...
DUK_LOCAL void duk__debug_handle_get_call_stack(duk_hthread *thr,
                                                duk_heap *heap)
{
    ...
    while(curr_thr != NULL)
    {
        for(curr_act = curr_thr->callstack_curr; curr_act != NULL;
            curr_act = curr_act->parent)
        {
...
DUK_LOCAL void duk__debug_handle_eval(duk_hthread *thr, duk_heap *heap)
{
    ...
    call_ret = duk_pcall_method_flags(thr, duk_get_top(thr) - (idx_func + 2),
                                      call_flags);
...
DUK_LOCAL void duk__debug_handle_apprequest(duk_hthread *thr, duk_heap *heap)
{
    ...
    if(heap->dbg_request_cb != NULL)
    {
        ...
        while(duk_debug_peek_byte(thr) != DUK_DBG_IB_EOM)
        {
            ...
            {
                DUK_D(DUK_DPRINT(
                    "failed to allocate space for request dvalue(s)"));
                goto fail;
            }
         ...
         */
        DUK_D(DUK_DPRINT("calling into AppRequest request_cb with nvalues=%ld, "
                         "old_top=%ld, top=%ld",
                         (long)nvalues, (long)old_top, (long)duk_get_top(thr)));
        nrets = heap->dbg_request_cb(thr, heap->dbg_udata, nvalues);
        DUK_D(DUK_DPRINT("returned from AppRequest request_cb; nvalues=%ld -> "
                         "nrets=%ld, old_top=%ld, top=%ld",
        ...
        if(nrets >= 0)
        {
            ...
            {
                DUK_D(DUK_DPRINT("AppRequest callback doesn't match value "
                                 "stack configuration, "
                                 "top=%ld < old_top=%ld + nrets=%ld; "
                                 "this might mean it's unsafe to continue!",
                                 (long)duk_get_top(thr), (long)old_top,
                                 (long)nrets));
                goto fail;
            }
        ...
        else
        {
            ...
            {
                DUK_D(DUK_DPRINT("request callback return value doesn't match "
    ...
    {
        DUK_D(
            DUK_DPRINT("no request callback, treat AppRequest as unsupported"));
                                       ...
                                       duk_heaphdr *hdr)
{
    ...
    case DUK_HTYPE_OBJECT:
    {
        ...
        duk_debug_write_uint(thr,
                             (duk_uint32_t)DUK_HOBJECT_GET_CLASS_NUMBER(h));
        duk_debug_write_heapptr(
            thr, (duk_heaphdr *)DUK_HOBJECT_GET_PROTOTYPE(heap, h));
        ...
        {
            duk_debug_write_uint(
                thr, (duk_uint32_t)DUK_HOBJECT_E_GET_FLAGS(heap, h, i));
            ...
            if(DUK_HOBJECT_E_SLOT_IS_ACCESSOR(heap, h, i))
            {
                ...
                duk_debug_write_heapptr(
                    thr, (duk_heaphdr *)DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)
                             ->a.get);
                duk_debug_write_heapptr(
                    thr, (duk_heaphdr *)DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)
                             ->a.set);
            ...
            else
            {
                ...
                duk__debug_write_tval_heapptr(
                    thr, &DUK_HOBJECT_E_GET_VALUE_PTR(heap, h, i)->v);
            }
        }
        ...
        for(i = 0; i < (duk_uint_fast32_t)DUK_HOBJECT_GET_ASIZE(h); i++)
        {
             ...
             */
            duk__debug_write_tval_heapptr(
                thr, DUK_HOBJECT_A_GET_VALUE_PTR(heap, h, i));
        }
        break;
    }
    case DUK_HTYPE_BUFFER:
    {
        ...
        duk_debug_write_buffer(thr,
                               (const char *)DUK_HBUFFER_GET_DATA_PTR(heap, h),
...
#endif
    DUK_HSTRING_FLAG_EXTDATA, 0 /* terminator */
};
DUK_LOCAL const char *const duk__debug_getinfo_hobject_keys[] = {
    "extensible",      "constructable",    "callable",
    "boundfunc",       "compfunc",         "natfunc",
    "bufobj",          "fastrefs",         "array_part",
    "strict",          "notail",           "newenv",
    "namebinding",     "createargs",       "have_finalizer",
    "exotic_array",    "exotic_stringobj", "exotic_arguments",
    "exotic_proxyobj", "special_call"
    /* NULL not needed here */
};
...
DUK_LOCAL duk_uint_t duk__debug_getinfo_hbuffer_masks[] = {
    DUK_HBUFFER_FLAG_DYNAMIC, DUK_HBUFFER_FLAG_EXTERNAL, 0 /* terminator */
};
...
DUK_LOCAL void duk__debug_getinfo_bitmask(duk_hthread *thr,
                                          const char *const *keys,
                                          duk_uint_t *masks, duk_uint_t flags)
{
    const char *key;
    duk_uint_t mask;
    ...
    for(;;)
    {
        ...
        DUK_DD(
            DUK_DDPRINT("inspect bitmask: key=%s, mask=0x%08lx, flags=0x%08lx",
                        key, (unsigned long)mask, (unsigned long)flags));
 ...
 */
DUK_LOCAL duk_bool_t duk__debug_getprop_index(duk_hthread *thr, duk_heap *heap,
                                              duk_hobject *h_obj,
                                              duk_uint_t idx)
{
    duk_uint_t a_size;
...
DUK_LOCAL void duk__debug_handle_get_heap_obj_info(duk_hthread *thr,
                                                   duk_heap *heap)
{
    duk_heaphdr *h;
    ...
    DUK_ASSERT(sizeof(duk__debug_getinfo_hstring_keys) / sizeof(const char *) ==
               sizeof(duk__debug_getinfo_hstring_masks) / sizeof(duk_uint_t) -
                   1);
    DUK_ASSERT(sizeof(duk__debug_getinfo_hobject_keys) / sizeof(const char *) ==
               sizeof(duk__debug_getinfo_hobject_masks) / sizeof(duk_uint_t) -
                   1);
    DUK_ASSERT(sizeof(duk__debug_getinfo_hbuffer_keys) / sizeof(const char *) ==
               sizeof(duk__debug_getinfo_hbuffer_masks) / sizeof(duk_uint_t) -
                   1);
    ...
    duk__debug_getinfo_prop_uint(thr, "heaphdr_flags",
                                 (duk_uint_t)DUK_HEAPHDR_GET_FLAGS(h));
    duk__debug_getinfo_prop_uint(thr, "heaphdr_type",
                                 (duk_uint_t)DUK_HEAPHDR_GET_TYPE(h));
#if defined(DUK_USE_REFERENCE_COUNTING)
    duk__debug_getinfo_prop_uint(thr, "refcount",
                                 (duk_uint_t)DUK_HEAPHDR_GET_REFCOUNT(h));
#endif
#if 0 /* pruned */
    ...
    case DUK_HTYPE_STRING:
    {
        ...
        h_str = (duk_hstring *)h;
        duk__debug_getinfo_bitmask(thr, duk__debug_getinfo_hstring_keys,
                                   duk__debug_getinfo_hstring_masks,
                                   DUK_HEAPHDR_GET_FLAGS_RAW(h));
        duk__debug_getinfo_prop_uint(
            thr, "bytelen", (duk_uint_t)DUK_HSTRING_GET_BYTELEN(h_str));
        duk__debug_getinfo_prop_uint(
            thr, "charlen", (duk_uint_t)DUK_HSTRING_GET_CHARLEN(h_str));
        duk__debug_getinfo_prop_uint(thr, "hash",
                                     (duk_uint_t)DUK_HSTRING_GET_HASH(h_str));
    ...
    case DUK_HTYPE_OBJECT:
    {
        ...
        duk__debug_getinfo_bitmask(thr, duk__debug_getinfo_hobject_keys,
        ...
        duk_debug_write_pointer(thr,
                                (void *)DUK_HOBJECT_GET_PROPS(heap, h_obj));
        duk__debug_getinfo_prop_uint(thr, "e_size",
                                     (duk_uint_t)DUK_HOBJECT_GET_ESIZE(h_obj));
        duk__debug_getinfo_prop_uint(thr, "e_next",
                                     (duk_uint_t)DUK_HOBJECT_GET_ENEXT(h_obj));
        duk__debug_getinfo_prop_uint(thr, "a_size",
                                     (duk_uint_t)DUK_HOBJECT_GET_ASIZE(h_obj));
        duk__debug_getinfo_prop_uint(thr, "h_size",
                                     (duk_uint_t)DUK_HOBJECT_GET_HSIZE(h_obj));
        ...
        if(DUK_HOBJECT_IS_ARRAY(h_obj))
        {
            ...
            duk__debug_getinfo_prop_uint(thr, "length",
                                         (duk_uint_t)h_arr->length);
            duk__debug_getinfo_prop_bool(thr, "length_nonwritable",
                                         h_arr->length_nonwritable);
        }
        ...
        if(DUK_HOBJECT_IS_NATFUNC(h_obj))
        {
            ...
            duk__debug_getinfo_prop_bool(
                thr, "varargs", h_fun->magic == DUK_HNATFUNC_NARGS_VARARGS);
            /* Native function pointer may be different from a void pointer,
             * and we serialize it from memory directly now (no byte swapping
             * etc).
             */
            duk__debug_getinfo_flags_key(thr, "funcptr");
            duk_debug_write_buffer(thr, (const char *)&h_fun->func,
                                   sizeof(h_fun->func));
        }
        ...
        if(DUK_HOBJECT_IS_DECENV(h_obj))
        {
            ...
            duk__debug_getinfo_prop_uint(thr, "regbase",
                                         (duk_uint_t)h_env->regbase_byteoff);
        }
        ...
        if(DUK_HOBJECT_IS_BUFOBJ(h_obj))
        {
            ...
            duk__debug_getinfo_prop_bool(thr, "is_typedarray",
                                         (duk_uint_t)h_bufobj->is_typedarray);
    ...
    case DUK_HTYPE_BUFFER:
    {
        ...
        h_buf = (duk_hbuffer *)h;
        duk__debug_getinfo_bitmask(thr, duk__debug_getinfo_hbuffer_keys,
                                   duk__debug_getinfo_hbuffer_masks,
                                   DUK_HEAPHDR_GET_FLAGS_RAW(h));
        duk__debug_getinfo_prop_uint(thr, "size",
                                     (duk_uint_t)DUK_HBUFFER_GET_SIZE(h_buf));
        duk__debug_getinfo_flags_key(thr, "dataptr");
        duk_debug_write_pointer(
            thr, (void *)DUK_HBUFFER_GET_DATA_PTR(thr->heap, h_buf));
        duk__debug_getinfo_flags_key(thr, "data");
...
DUK_LOCAL void duk__debug_handle_get_obj_prop_desc(duk_hthread *thr,
                                                   duk_heap *heap)
{
    duk_heaphdr *h;
    duk_hobject *h_obj;
    ...
    if(duk_hobject_get_own_propdesc(thr, h_obj, h_key, &desc, 0 /*flags*/))
    {
        ...
        duk_debug_write_reply(thr);
        rc =
            duk__debug_getprop_index(thr, heap, h_obj, (duk_uint_t)virtual_idx);
        DUK_ASSERT(rc == 1);
        DUK_UNREF(rc);
        duk_debug_write_eom(thr);
    }
                                                   ...
                                                   duk_bool_t no_block)
{
    ...
    DUK_D(DUK_DPRINT("process debug messages: read_cb=%s, no_block=%ld, "
                     ...
                     (long)no_block, (long)thr->heap->dbg_detaching,
    ...
    for(;;)
    {
        ...
        while(thr->heap->dbg_read_cb == NULL && thr->heap->dbg_detaching)
        {
            ...
            DUK_D(DUK_DPRINT("after detach2 (and possible reattach): "
                             "dbg_read_cb=%s, dbg_detaching=%ld",
                             thr->heap->dbg_read_cb ? "not NULL" : "NULL",
        ...
        {
            DUK_D(DUK_DPRINT("debug connection broken (and not detaching), "
        ...
        {
            if(!user_lock_debugstream(1, 0))
            {
                /* Note: peek cannot currently trigger a detach
                 * so the dbg_detaching == 0 assert outside the
                 * loop is correct.
                 */
                DUK_D(DUK_DPRINT("processing debug message, peek indicated no "
                                 "data, stop processing messages"));
                break;
            }
            DUK_D(DUK_DPRINT("processing debug message, peek indicated there "
                             "is data, handle it"));
        }
        else
        {
            ...
            if(user_lock_debugstream(1, 1) == 2 && thr->sent_fatal_throw)
                duk_debug_send_throw(thr, 1); // resend throw if new debugstream
        }
...
DUK_INTERNAL void duk_debug_halt_execution(duk_hthread *thr,
...
duk_debug_remove_breakpoint(duk_hthread *thr, duk_small_uint_t breakpoint_index)
{
    duk_heap *heap;
    duk_hstring *h;
    duk_breakpoint *b;
    ...
    {
        DUK_D(DUK_DPRINT("invalid breakpoint index: %ld",
    ...
    move_size = sizeof(duk_breakpoint) *
                (heap->dbg_breakpoint_count - breakpoint_index - 1);
    if(move_size > 0)
    {
...
DUK_EXTERNAL void duk_copy_breakpoints(duk_hthread *from_ctx,
                                       duk_hthread *to_ctx)
{
    duk_heap *from = from_ctx->heap;
    duk_heap *to = to_ctx->heap;
    duk_idx_t entry_top;
    int i;

    memcpy(to->dbg_breakpoints, from->dbg_breakpoints,
           sizeof(from->dbg_breakpoints));
    to->dbg_breakpoint_count = from->dbg_breakpoint_count;

    entry_top = duk_get_top(to_ctx);
    for(i = 0; i < to->dbg_breakpoint_count; i++)
    {
        duk_hstring *h = from->dbg_breakpoints[i].filename;
        duk_push_lstring(
            to_ctx, (h != NULL ? (const char *)DUK_HSTRING_GET_DATA(h) : ""),
            (h != NULL ? (duk_size_t)DUK_HSTRING_GET_BYTELEN(h) : 0));

        to->dbg_breakpoints[i].filename = h = duk_require_hstring(to_ctx, -1);
        DUK_HSTRING_INCREF(to, h);
    }
    duk_set_top(to_ctx, entry_top);
}
...
duk_bool_t duk_debug_is_paused_extern(duk_hthread *ctx)
{
    return (DUK_HEAP_HAS_DEBUGGER_PAUSED(ctx->heap) != 0);
}
...
DUK_INTERNAL void duk_debug_set_paused(duk_heap *heap)
{
    ...
    else
    {
        ...
        DUK_ASSERT(heap->ms_running ==
                   0); /* debugger can't be triggered within mark-and-sweep */
        heap->ms_running =
            1; /* prevent mark-and-sweep, prevent refzero queueing */
