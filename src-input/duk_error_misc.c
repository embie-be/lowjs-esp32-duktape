...
DUK_INTERNAL void duk_err_check_debugger_integration(duk_hthread *thr)
{
    ...
    if(!duk_debug_is_attached(thr->heap) || thr->heap->dbg_processing ||
       thr->heap->lj.type != DUK_LJ_TYPE_THROW || thr->heap->creating_error)
    {
        DUK_D(DUK_DPRINT(
            "skip debugger error integration; not attached, debugger "
            "processing, not THROW, or error thrown while creating error"));
        return;
    }
    ...
    if(DUK_TVAL_IS_OBJECT(tv_obj) &&
       DUK_TVAL_GET_OBJECT(tv_obj) == thr->builtins[DUK_BIDX_DOUBLE_ERROR])
    {
        DUK_D(DUK_DPRINT(
    ...
    DUK_ASSERT(DUK_TVAL_IS_UNDEFINED(
        &thr->heap->lj.value2)); /* Always for THROW type. */
        ...
        if(uncaught)
        {
            if(thr->heap->dbg_pause_flags & DUK_PAUSE_FLAG_UNCAUGHT_ERROR)
            {
                if(code_print_error())
                {
    	            ...
    	            DUK_D(DUK_DPRINT(
        	            "throw with debugger attached, report to client"));
